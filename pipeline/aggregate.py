...
df["zscore_allowed_amt"] = df.groupby("provider_id")["avg_allowed_amt"].transform(lambda s: zscore(s, nan_policy='omit'))
else:
...
fig = px.line(df.sort_values("date"), x="date", y="avg_allowed_amt", color="provider_id", title="Avg Allowed Amount by Provider (Daily)")
